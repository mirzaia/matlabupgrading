...
Maxit   = 10;                     %iterasi
el      = 0.1;                    %elatism
Pc      = 0.75;                   %probabilitas crossover
Pm      = 0.0005;                 %probabilitas mutasi
Nbit    = 20;                     %jumlah bit
%Constrain
...
%Mutasi
    ...
    for i = 1:Npop
        ...
        DatafitY = [DatafitY;fitness];
        %for po = 0:population;
        
      %% BalikData=1./DatafitY;
%         hY = plot (BalikData);
%         hold on
...
%         hold on
    ...
    efitnessmax = [efitnessmax;fitness];
%     efitnessmaxplot = [efitnessmaxplot;fitness];
balikdata=1./efitnessmax;
...
figure(gcf)
title('Grafik Nilai Minimum GA','color','b')
xlabel('Iterasi')
ylabel('Nilai Fungsi Obyektif')
hold on
plot(balikdata, 'DisplayName', 'efitnessmax', 'YDataSource', 'efitnessmax');
 hold on
